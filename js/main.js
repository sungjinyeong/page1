...
function setupHamburgerToggle() {
  ...
  btns.forEach(btn => btn.addEventListener('click', () => {
    const h = btn.closest('.header');
    if (!h || !h.classList.contains('anchors')) return;
...
function hideHeaderOnSec9() {
  ...
  const s9 = document.querySelector('.sec9');
  if (!h || !s9) return;
...
function setupSec3HistoryList() {
  const el = document.querySelector('.sec3 .history');
  const items = document.querySelectorAll('.sec3 .history .his_tt');
  ...
  new IntersectionObserver(es => es.forEach(e => {
    if (e.isIntersecting) {
      clearTimeout(lt);
      tts.forEach(clearTimeout); tts = [];
      const d = window.innerWidth <= 768 ? 1500 : 2000;
      if (d > 1500) el.classList.add('line-animate');
      lt = setTimeout(() => items.forEach((x, i) => tts.push(setTimeout(() => x.classList.add('visible'), i * 800))), d);
    } else {
      clearTimeout(lt); tts.forEach(clearTimeout); tts = [];
...
function setupSec4to6TextAnimations() {
  const cfg = [
    { sel: '.sec4', ech: ['.h2q', '.h3q', '.textq'] },
    { sel: '.sec5 .title', ech: ['.h2q', '.h3q', '.textq'] },
    { sel: '.sec6 .title', ech: ['.h2q', '.h3q', '.textq'] }
  ];
  cfg.forEach(({ sel, ech }) => {
    const root = document.querySelector(sel);
    if (!root) return;
    const els = ech.map(s => root.querySelector(s)).filter(Boolean);
    let iks = [];
    els.forEach(x => { x.style.opacity = '0'; x.style.transform = 'translateY(20%)'; x.style.transition = 'opacity 1s ease,transform 1s ease'; });
    new IntersectionObserver(es => es.forEach(e => {
      if (e.isIntersecting) {
        els.forEach((x, i) => iks.push(setTimeout(() => { x.style.opacity = '1'; x.style.transform = 'translateY(0)' }, i * 800)));
      } else {
        iks.forEach(clearTimeout); iks = [];
        els.forEach(x => { x.style.opacity = '0'; x.style.transform = 'translateY(20%)'; });
      }
...
function setupSec6RoundList() {
  ...
  if (!sec6 || !items.length) return;
  let st, tts = [];
  items.forEach(x => x.classList.remove('visible'));
  new IntersectionObserver(es => es.forEach(e => {
    if (e.isIntersecting) {
      clearTimeout(st); tts.forEach(clearTimeout); tts = [];
      st = setTimeout(() => items.forEach((x, i) => tts.push(setTimeout(() => x.classList.add('visible'), i * 500))), 2000);
    } else {
      clearTimeout(st); tts.forEach(clearTimeout); tts = [];
      items.forEach(x => x.classList.remove('visible'));
    }
...
function setupServiceListAnimation() {
  const items = document.querySelectorAll('.service_list li');
  ...
  new IntersectionObserver(es => es.forEach(e => {
    if (e.isIntersecting) {
      clearTimeout(st); tts.forEach(clearTimeout); tts = [];
      st = setTimeout(() => items.forEach((x, i) => tts.push(setTimeout(() => { x.style.opacity = '1'; x.style.transform = 'translateY(0)' }, i * 500))), 1500);
    } else {
      clearTimeout(st); tts.forEach(clearTimeout); tts = [];
      items.forEach(x => { x.style.opacity = '0'; x.style.transform = 'translateY(30px)'; });
    }
...
function setupSponSlider() {
  ...
  function init() {
    ...
    imagesLoaded('.row1 .track', () => $('.row1 .track').addClass('slider1').bxSlider({ slideSelector: 'img', minSlides: 5, maxSlides: 20, moveSlides: 1, slideWidth: 150, slideMargin: 20, ticker: true, speed: speed(85000), useCSS: false }));
    imagesLoaded('.row2 .track', () => $('.row2 .track').addClass('slider2').bxSlider({ slideSelector: 'img', minSlides: 5, maxSlides: 20, moveSlides: 1, slideWidth: 150, slideMargin: 20, ticker: true, speed: speed(60000), useCSS: false }));
  }
  init();
